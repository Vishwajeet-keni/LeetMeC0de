...
- 📝 Adds a per-problem `README.md` with the problem title, difficulty, and statement (written once — not re-written on repeat language submissions)
- 🈁 Multiple language solutions for the same problem coexist side by side (e.g. `two-sum.py` and `two-sum.cpp` in the same folder)
- ⏭️ Skips committing if the code is byte-identical to what's already on GitHub — no noisy no-op commits on re-submission
- 🔁 One automatic retry on the GraphQL metadata lookup before falling back to `Unknown` difficulty
- 🕘 Sync history in the popup — last 10 syncs with status, difficulty tag, and timestamp
...
- 🎨 Obsidian-and-platinum design system across the icon, popup, and Settings page
...
- **Content script:** `scripts/leetcode-detector.js`, injected into `https://leetcode.com/problems/*`
- **Page-context hook:** `scripts/injected.js`, loaded via `web_accessible_resources` to patch `window.fetch` inside the page's own JS context
- **Backing APIs:** GitHub REST API (`api.github.com/repos/.../contents/...`) and LeetCode's public GraphQL endpoint (`leetcode.com/graphql/`) for difficulty and problem-statement lookup
...
3. Click **Test Connection** → confirm the status dot turns green
4. Save
...
- **Include README per problem:** checkbox in Settings — writes a `README.md` once per problem folder, skipped on subsequent language syncs for the same problem
...
| File | Role | Behavior |
|---|---|---|
| `manifest.json` | Extension manifest | Declares permissions, content script, background worker, popup, options page, icons |
| `scripts/leetcode-detector.js` | Content script (isolated world) | Injects `scripts/injected.js` into the page; relays `postMessage` events to the background worker via `chrome.runtime.sendMessage` |
| `scripts/injected.js` | Page-context script | Patches `window.fetch` to watch `/submit/` calls; polls the result-check endpoint until a verdict; fetches problem metadata via GraphQL with one retry |
| `background.js` | Service worker | Receives accepted-submission messages; checks for unchanged code/README before writing; builds the file path; commits to GitHub via the REST API; maintains sync history |
| `popup/popup.html/js/css` | Toolbar popup | Shows connection status, enable/disable toggle, and last 10 syncs with status |
| `options/options.html/js/css` | Settings page | Token/owner/repo/branch fields, live connection status dot, Test Connection button, folder-structure preference |
| `icons/` | Extension icon | `icon.svg` source plus rasterized `icon16.png`, `icon48.png`, `icon128.png` |
...
LeetCode page (fetch/XHR)
  ...
  → on Accepted: injected.js fetches title/difficulty/problem statement via leetcode.com/graphql/ (1 retry on failure)
  → postMessage → leetcode-detector.js
  → chrome.runtime.sendMessage → background.js
  → background.js builds path (Easy|Medium|Hard/<slug>/<slug>.<ext>)
  → GitHub REST API: GET contents (check for existing sha + compare content) → PUT contents (commit, only if changed)
  → GET contents for README.md (skip write if it already exists)
  → sync history entry pushed to chrome.storage.local
```
...
- Metadata lookup depends on LeetCode's public, **undocumented** GraphQL schema (`question(titleSlug) { title difficulty content }`) — this endpoint can change shape without notice.
...
**Nothing happens on submit** — open the LeetCode page's own DevTools console; confirm `window.__leetmec0de_lastSubmission` populates after clicking Submit. If it's `undefined`, the submit hook never fired — check the Network tab for the actual `/submit/` request URL and compare against the regex in `scripts/injected.js`.
...
**Folder structure shows `Unknown/<slug>` instead of `Easy/Medium/Hard`** — the GraphQL metadata lookup either failed twice (initial attempt + retry) or hadn't resolved yet when the submission was committed. Check the service worker/page console for `🎯 Meta for <slug> = ...`; if it's missing or errors, LeetCode's GraphQL schema may have shifted.

**Resubmitting doesn't create a new commit** — this is expected behavior, not a bug. If the code is byte-identical to what's already on GitHub, the sync is intentionally skipped (see popup history: `"No changes — skipped"`).
...
```
LeetMeC0de/
├── manifest.json          # Manifest V3 config: permissions, content script, background worker, icons
├── background.js          # Service worker — GitHub REST API calls, diff-checking, path building, sync history
├── scripts/
│   ├── leetcode-detector.js   # Content script (isolated world) — bridges page and extension
│   └── injected.js           # Page-context script — fetch/XHR patching, verdict polling, metadata retry
├── popup/
│   ├── popup.html
│   ├── popup.js
│   └── popup.css
...
Files are grouped by UI area (`scripts/`, `popup/`, `options/`) rather than by technical role — a folder exists only once a feature has enough files to warrant it, same philosophy as [hypr_bar](https://github.com/Vishwajeet-keni/hypr_bar).
...
- **Chrome Web Store listing** — currently install-as-unpacked only; needs a privacy policy page and store screenshots.
- **`KNOWN_ISSUES.md`** — track the two real LeetCode-side breakages already hit during development (GraphQL endpoint migration, extension-context-invalidation on reload) so they're not rediscovered from scratch later.
- **Retroactive re-organization** — solutions committed before the difficulty-folder fix landed (into a legacy `Solved/` or `Unknown/` folder) needed manual cleanup once; no automated migration exists if it happens again.
